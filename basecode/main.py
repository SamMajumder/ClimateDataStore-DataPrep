...
if not url or not key:
    print("\n❌ Missing CDSAPI credentials!")
    print("Please set the environment variables 'CDSAPI_URL' and 'CDSAPI_KEY'.")
    print("You can define them in your local .env, .secrets file, or GitHub Actions secrets.\n")
    exit(1)  # Exit cleanly with status 1 (error)

shapefile_folder = "vermont"
## find the first shapefile within the folder 
shapefile_list = glob.glob(os.path.join(shapefile_folder,"*shp"))

if not shapefile_list:
    raise FileNotFoundError(f"❌ No shapefiles found inside {shapefile_folder}")

shapefile = shapefile_list[0]  

print(f"[INFO] Found shapefile: {shapefile}")
...
month=["01","02"]
data_format="netcdf"
download_folder="vermont_monthly"

bronze_output_folder="vermont_monthly_bronze"

silver_output_folder = "vermont_monthly_silver"




## This kicks off the process
def main(shapefile=shapefile,
         url = url,
         key = key,
         dataset = dataset,
